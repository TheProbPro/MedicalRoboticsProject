...
def find_valid_trajectory(robot, target_pose, q_start):
    ...
    while True:
        ...
        q, success, iter, searches, residual = robot.ik_LM(target_flange_pose, q0=rtde_rec.getTargetQ())#robot.q)
...
def execute_trajectory(robot, trajectory, admittanceController, box_plane, box_size, dt):
    ...
    f = np.zeros(3)
    mu = np.zeros(3)
    quat_init = SE3(robot.fkine(rtde_rec.getTargetQ())).UnitQuaternion()
    ...
    for q in trajectory.q:
        ...
        target_flange_pose = target_pose * robot.tool.inv()
        # Compute the inverse kinematics
        q, success, iter, searches, residual = robot.ik_LM(target_flange_pose, q0=rtde_rec.getTargetQ(), end='tool0')
        # Check if the IK solution is valid
        if not success:
            raise RuntimeError("IK failed to find a solution.")
    ...
    while True:
        ...
        current_tool_pose = robot.fkine(rtde_rec.getTargetQ()) * robot.tool
        # Get the target pose
        target_pose = SE3(current_tool_pose.t) * SE3(SO3.Rx(np.pi)) * SE3(0, 0, 0.01)
        # Find a valid trajectory to the target pose
        traj = find_valid_trajectory(robot, target_pose, rtde_rec.getTargetQ())
...
if __name__ == '__main__':
    ...
    while optimizer.get_number_of_samples() < 20:
        ...
        if i < 5:
            ...
            traj = find_valid_trajectory(robot, xyToRobotPose(samples[i], robot), rtde_rec.getTargetQ())
        ...
        # For the remaining x samples
            ...
            # find valid trajectory
            traj = find_valid_trajectory(robot, xyToRobotPose(next_sample, robot), rtde_rec.getTargetQ())
            # execute trajectory and get stiffness
            stiffness = execute_trajectory(robot, traj, adm_controller, box_plane, box_size, dt)
